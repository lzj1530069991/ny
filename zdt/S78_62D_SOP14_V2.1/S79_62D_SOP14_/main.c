...
#define L1OUT	IOSTB &= 0xDE
#define L2OUT	IOSTA &= 0xFE
#define L3OUT	IOSTA &= 0xFD

#define L1IN	IOSTB |= 0x21
#define L2IN	IOSTA |= 0x01
#define L3IN	IOSTA |= 0x02

#define L1H		PORTB |= 0x21
#define L2H		PORTA |= 0x01
#define L3H		PORTA |= 0x02

#define L1L		PORTB &= 0xDE
#define L2L		PORTA &= 0xFE
#define L3L		PORTA &= 0xFD
...
void keyCtr()
{
	...
	if(kclick == 1)
	{
		count500ms = 0;
		ledLightTime = 0;
		if((workStep == 0 && pwStep == 0) || closeFlag)
		{
			checkBatAD();
			return;
		}
...
void chrgCtr()
{
	if(PORTA & 0x08)
	{
		...
		else
		{
			...
			if(count200ms > 5)
			{
					if(R_AIN4_DATA <= 23 && R_AIN4_DATA >=subMin)
					{
						addTime = subTime = 0;
					}	
					if(R_AIN4_DATA > 23)
