...
#define L2OUT	IOSTA &= 0xFD
#define L3OUT	IOSTA &= 0xFE

#define L1IN	IOSTB |= 0x02
#define L2IN	IOSTA |= 0x02
#define L3IN	IOSTA |= 0x01

#define L1H		PORTB |= 0x02
#define L2H		PORTA |= 0x02
#define L3H		PORTA |= 0x01

#define L1L		PORTB &= 0xFD
#define L2L		PORTA &= 0xFD
#define L3L		PORTA &= 0xFE
...
__sbit longPressFlag2 = Status:4;
...
void main(void)
{
	...
	while(1)
	{
	    ...
	    else if(workStep > 0 && tempDuty >= 45)
	    {
	    	if(cDuty > tempDuty)
	    	{
	    		cDuty--;
		...
		if(workStep == 0 && keyCount == 0 && (PORTA & 0x04) == 0 && ledLightTime == 0 && ledDeadTime == 0 && shanshuoTime == 0)
		{
			if(++sleepTime > 20)
			{
				gotoSleep();
...
{
	 DISI();
	IOSTA = C_PA7_Input | C_PA5_Input  | C_PA3_Input | C_PA2_Input ;
	IOSTB = C_PB0_Input;
	PORTB = 0x00;
	PORTA = 0x00;
	LedInput();
