...
//	TMR1 = 249;							//频率为110K
	//PWM2DUTY = 0x08;				// 		
	T1CR2	 = C_PS1_Div128 | C_TMR1_ClkSrc_Inst;	// Prescaler 1:1 , Timer1 clock source is instruction clockF
...
void keyCtr()
{
	...
	if(kclick == 1)
    {
    	...
    	{
    		T1CR2	 = C_PS1_Div16 | C_TMR1_ClkSrc_Inst;
    	}
    	else if(workStep == 5)
    	{
    		T1CR2	 = C_PS1_Div8 | C_TMR1_ClkSrc_Inst;
    	}
    	else if(workStep == 6)
    	{
    		T1CR2	 = C_PS1_Div2 | C_TMR1_ClkSrc_Inst;
    	}
    	if(workStep > 0)
    		pwmInit();
    }
