...
void main(void)
{
	...
	while(1)
	{
		...
		if(workStep == 0 && keyCount == 0 && (PORTA & 0x40) == 0 && ledLightTime == 0 && ledDeadTime == 0 && shanshuoTime == 0)
		{
			if(++sleepTime > 20)
			{
				gotoSleep();
...
{
	 DISI();
	IOSTA = C_PA7_Input | C_PA5_Input | C_PA4_Input | C_PA6_Input | C_PA2_Input | C_PA1_Input | C_PA0_Input;
	IOSTB = 0x00;
	PORTB = 0x00;
	PORTA = 0x00;
	LedInput();
...
void chrgCtr()
{
	if(PORTA & 0x40)
	{
		...
		else
		{
			...
			IOSTB &= 0xF7;
			fullCount = 0;
			if(count500ms == 0 && pwStep <= 6 && lockLedStep < 5)
			{
