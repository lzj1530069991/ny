...
//				waitTime--;
		if(pwFlag)
		{
			...
			if((++count250ms >= 35) || revCountFlag || irStep)
	    	{
...
void checkBat()
{
    ...
    else
    {
    	u16t tempValue = 0;
    	fullTime = 0;
    	if(R_Quarter_VDD_DATA >= 2050)
    	{
    		tempValue = (R_Quarter_VDD_DATA - 2050) + 75;
    	}
    	else if(R_Quarter_VDD_DATA >= 1998)
    	{
    	...
    	{
    		tempValue = (R_Quarter_VDD_DATA - 1550)/16;
    	}
    	else
    	{
    		tempValue = 0;
