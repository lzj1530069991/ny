...
typedef float            fp64;                    /* 双精度浮点数（64位长度）                 */
...
typedef struct {  
   ...
   /*read interrupt*/
   void (*ms_read_int_status)(uint8_t *low_g, uint8_t *high_g);   
   /*clear interrupt*/
   ...
   /*read device status*/
   uint8_t (*ms_query_gesture)(uint16_t ms_gest_type);
   uint8_t (*ms_get_sensor_params)(uint16_t ms_params_type, MotionSensorQueryStruct *ms_params);
   uint8_t (*ms_set_sensor_params)(uint16_t ms_params_type, uint32_t ms_params);
