...
void main(void)
{
	...
	while(1)
	{
		...
		if(workStep == 0 && keyCount == 0 && (PORTA & 0x20) == 0 && ledLightTime == 0)
		{
			if(++sleepTime > 300)
			{
				gotoSleep();
...
{	
        ...
        if(R_AIN4_DATA >= 420)
       	...
       	else if(R_AIN4_DATA > 395)
       	{
			...
			//充电充不满时候
			if(chrgTime > 0 && R_AIN4_DATA > 415 && chrgTime > 600)
			{
					chrgTime = 600;
					pwStep = 9;
			}
