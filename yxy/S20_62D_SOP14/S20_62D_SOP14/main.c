...
//		}
		...
		chrgCtr();
		if(workStep == 0 && keyCount == 0 && ((PORTA & 0x20) == 0))
		{
			if(++sleepTime > 400)
			{
				gotoSleep();
...
{	
        ...
        R_AIN4_DATA = R_AIN4_DATA - 1145;

        if(R_AIN4_DATA >= 422)
       	{
       		pwStep = 5;
       	}
       	else if(R_AIN4_DATA > 350)
       	{
			fullCount = 0;
			if(pwStep < 4)
      		{
			...
			else if(chrgTime == 0 && R_AIN4_DATA < 396)
			{
					pwStep = 4;
			}
		
			//充电充不满时候
			if(chrgTime > 0 && R_AIN4_DATA > 395 && chrgTime > 600)
			{
					chrgTime = 600;
					pwStep = 5;
			}
